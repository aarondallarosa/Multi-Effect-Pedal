...
int main(){
    ...
    // Select which pedal to use
    unsigned int Pedal;
    std::string Pedal_list[2] = {"TR2", "DigitalDelay"};
    std::cout << "Select which pedal to use:\n";
    for (int i = 0; i < 2; ++i)
        std::cout << i << ": " << Pedal_list[i] << std::endl;
        std::cin >> Pedal; // User enters 0 or 1
    if (Pedal == 0) {
        // Run TR2 pedal code
    } else if (Pedal == 1) {
    // Run DigitalDelay pedal code
    } else {
    ...
    SNDFILE* outfile = sf_open("trem.wav", SFM_WRITE, &sfoutinfo);
    if (!outfile) {
